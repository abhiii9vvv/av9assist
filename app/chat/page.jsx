...
export default function ChatPage() {
  ...
  const exportToPDF = (title, timestamp) => {
...
<body>
  ...
  <div class="meta">
    ...
    <div>Total Messages: ${conversationMessages.length}</div>
  </div>
  ${conversationMessages.map(msg => {
    const time = new Date(msg.timestamp).toLocaleString()
    const senderLabel = msg.sender === 'user' ? 'You' : 'AI Assistant'
    const messageClass = msg.sender === 'user' ? 'user' : 'ai'
    return `
  ...
  <div class="footer">
    Generated by av9Assist - ${timestamp}
  </div>
</body>
</html>
    `
    ...
    if (printWindow) {
      ...
      a.href = url
      a.download = `${title.replace(/[^a-z0-9]/gi, '_')}_${timestamp}.html`
  ...
  const exportConversation = (format) => {
    ...
    if (format === 'pdf') {
      ...
      exportToPDF(conversationTitle, timestamp)
    } else if (format === 'json') {
      const data = {
