...
class AIProvider {
   ...
   */
  async getAIResponse(message, providerOrder = null, context = null) {
    const providers =
      providerOrder || (process.env.PROVIDERS_ORDER || "gemini,sambanova,openrouter,huggingface").split(",")

    for (const providerName of providers) {
      const provider = providerName.trim()
      if (!this.providers[provider] || !this.configs[provider].apiKey) {
        console.log(`⚠️ Skipping ${provider} - not configured or missing API key`)
        continue
      }

      try {
        console.log(`🔄 Trying ${provider}...`)
        const response = await this.providers[provider](message, context)
        if (response) {
   ...
   * - Each message is provided as { role, parts: [{ text }] }.
   */
  async callGemini(message, context) {
    const config = this.configs.gemini
    const url = config.url.replace("{model}", config.model).replace("{key}", config.apiKey)
    ...
    if (Array.isArray(context)) {
      for (const m of context.slice(-8)) {
        ...
        contents.push({ role: mappedRole, parts: [{ text: String(m.content) }] })
      }
    }
   ...
   */
  async callHuggingFace(message, context) {
    const config = this.configs.huggingface
    const url = config.url.replace("{model}", config.model)

    // Prepend a light-weight context prefix since HF text-generation-inference models often take raw text
    let prefix = ""
