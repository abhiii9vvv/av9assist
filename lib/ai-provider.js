...
const PollinationsProvider = require("./pollinations-provider")

// Environment variables loaded from .env.local by Next.js automatically
...
class AIProvider {
  constructor() {
    ...
    this.providers = {
      ...
      pollinations: this.callPollinations.bind(this),
    }
    ...
    this.configs = {
      gemini: {
        ...
        apiKey3: process.env.GOOGLE_API_KEY_3,
    ...
    console.log('🔑 AIProvider initialized with keys:')
  console.log('  GOOGLE_API_KEY:', this.configs.gemini.apiKey ? '✅ SET' : '❌ MISSING')
  console.log('  GOOGLE_API_KEY_2:', this.configs.gemini.apiKey2 ? '✅ SET' : '❌ MISSING')
  console.log('  GOOGLE_API_KEY_3:', this.configs.gemini.apiKey3 ? '✅ SET' : '❌ MISSING')
    console.log('  SAMBANOVA_API_KEY:', this.configs.sambanova.apiKey ? '✅ SET' : '❌ MISSING')
    console.log('  OPENROUTER_API_KEY:', this.configs.openrouter.apiKey ? '✅ SET' : '❌ MISSING')
    console.log('  POLLINATIONS:', this.configs.pollinations.enabled ? '✅ ENABLED (No API key required)' : '❌ DISABLED')
  ...
  async getAIResponse(message, providerOrder = null, context = null, imageData = null) {
    const providers =
      providerOrder || (process.env.PROVIDERS_ORDER || "gemini,sambanova,openrouter,pollinations").split(",")
    ...
    for (const providerName of providers) {
      ...
      if (provider === 'pollinations') {
        if (!this.configs[provider].enabled) {
          console.log(`⚠️ Skipping ${provider} - provider disabled`)
          continue
        }
      }
      // For Gemini, check if either API key exists
        ...
        if (
          !this.configs[provider].apiKey &&
          !this.configs[provider].apiKey2 &&
          !this.configs[provider].apiKey3
        ) {
  ...
  const apiKeys = [config.apiKey, config.apiKey2, config.apiKey3].filter(Boolean)
    let lastError = null
    ...
    // Note: Gemini v1 API doesn't support system_instruction field
    // System context is already included in the contents array from the context parameter
    ...
    const response = await this.makeRequest(url, {
      ...
      timeoutMs: 60000, // 60 second timeout for vision API
    })
   ...
   */
  async callPollinations(message, context) {
    try {
      const config = this.configs.pollinations
      
      console.log('🌸 Calling Pollinations.AI...')
      
      // Use the Pollinations provider instance
      const response = await this.pollinations.generateText(message, context, {
        model: config.model,
      })
  ...
  makeRequest(url, options) {
    return new Promise((resolve, reject) => {
      ...
      const reqOptions = {
        ...
        timeout: Number.isFinite(options.timeoutMs) ? options.timeoutMs : 60000, // 60 second default for HTTP timeout
      }
